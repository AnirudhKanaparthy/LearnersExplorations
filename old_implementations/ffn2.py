        ...
        for j in range(l + 1):
            ...
            for i in range(len(trainX)):
                ...
                if j == 0:
                    ...
                    d_c_a = np.sum((network['W'][l - j + 1] * d_a_z * d_c_a).T, axis=1).reshape(network['W'][l - j + 1].shape[1], 1)
...
def main():
    ...
    x = np.asarray([0, 1]).reshape(2, 1)
    res = forward_prop(network, x)
    print(f'Result:\n{res}')
